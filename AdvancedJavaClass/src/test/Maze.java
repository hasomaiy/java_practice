...
public class Maze
{
    ...
    public class Point {
        ...
        public Point(int x1, int y1, int g1) {
            x=x1;
            y=y1;
    ...
    private void search() {
        ...
        Stack<Point> explored = new Stack<>();
        boolean isBacktrack = false;
        
        s.push(new Point(1,n));
        ...
        while (true) {
            ...
            if (x==n && y==1){ System.out.println("\n\n*****SUCCESS******!!!");break;}

            if (!data.south[x][y]) {
            	Point node = new Point(x,y-1);
                if (!visited[x][y-1])
                    s.push(node);
                               
            }
            if (!data.east[x][y]) {
            	Point node = new Point(x+1,y);
                if (!visited[x+1][y])
                    s.push(node);
